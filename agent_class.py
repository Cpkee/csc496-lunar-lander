    ...
    def train(self, environment, verbose=True, model_filename=None, training_filename=None):
        ...
        for n_episode in range(self.n_episodes_max):
            ...
            with torch.no_grad():
                ...
                new_logprobs = self.actor(states).log_prob(actions)
                kl_div = (old_logprobs - new_logprobs).mean().item()
                
                # Clip fraction (how often we're hitting the clip limit)
            ...
            training_diagnostics['policy_losses'].append(policy_loss.item())
            training_diagnostics['value_losses'].append(value_loss.item())
            training_diagnostics['entropies'].append(entropy.mean().item())
            training_diagnostics['kl_divergences'].append(kl_div)
            training_diagnostics['clip_fractions'].append(clip_fraction)
            training_diagnostics['explained_variances'].append(explained_var)
            training_diagnostics['learning_rates_actor'].append(
                self.optimizer_actor.param_groups[0]['lr']
            )
            training_diagnostics['learning_rates_critic'].append(
        ...
        results['diagnostics'] = training_diagnostics
        return training_results
